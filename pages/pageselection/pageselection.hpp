...
class MenuPage : public Page {
    ...
    public:
        ...
        MenuPage(std::string name, const std::vector<std::string>& page_names) : Page(name), selected_idx(0) {
            init_page(page_names);
        }

        MenuPage(std::string name, const std::vector<std::string>& page_names, size_t selected_idx) : Page(name), selected_idx(selected_idx), initial_idx(selected_idx) {
            init_page(page_names);
        }

        MenuPage(std::string name, const std::vector<std::string>& page_names, const std::string& selected_name) : Page(name) {
            std::vector<std::string>::const_iterator it = std::find(page_names.begin(), page_names.end(), selected_name);
